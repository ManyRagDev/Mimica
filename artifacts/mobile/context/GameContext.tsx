...
function buildTurn(
  ...
  if (!word) {
    ...
    const fallback = getRandomWord(state.settings.selectedCategories, [], difficulty);
...
function gameReducer(state: GameState, action: GameAction): GameState {
  switch (action.type) {
    ...
    case 'SKIP_WORD': {
      ...
      ) ?? getRandomWord(state.settings.selectedCategories, [...state.usedWordIds, word.id]);
...
export function GameProvider({ children }: { children: React.ReactNode }) {
  ...
  const proceedFromRoulette = useCallback(() => {
    if (game.settings.enableHotPotato) {
      dispatch({ type: 'PROCEED_TO_MODIFIER' });
